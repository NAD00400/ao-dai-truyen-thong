...
export async function GET(req: Request, { params }: { params: { madonhang: string } }) {
  try {
    ...
    const giaohang = await prisma.giao_hang.findFirst({
      where: { ma_don_hang : madonhang },
      select: { 
        sdt_giao_hang: true,
        phuong_giao_hang: true,
        quan_giao_hang: true,
        tinh_giao_hang: true ,
        duong_giao_hang:true,
    ...
    //   select: { ma_san_pham_dat_may: true, ten_san_pham: true ,gia_tien:true},
    // });
    ...
    //     price: product?.gia_tien || "UNKNOWN",
    //   };
    // });
